...
class TestClipLayers(unittest.TestCase):
	...
	mask_polygon = Polygon([(0,0),(0,10),(10 ,10),(10,0)])
	...
	def test_clip_layers(self):
		...
		self.assertEqual(type(result), dict)
		lines_clipped_correctly = self.mask_gdf['geometry'].contains(gpd.GeoSeries(result['lines']['geometry'].all())).iloc[0]
		self.assertEqual(lines_clipped_correctly, True)
		polygons_clipped_correctly = self.mask_gdf['geometry'].contains(gpd.GeoSeries(result['polygons']['geometry'].all())).iloc[0]
